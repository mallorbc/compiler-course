...
public:
    ...
    bool parse_variable_declaration(bool is_global, token procedure_name_token);
    ...
    bool parse_type_mark(token procedure_name_token, int context);
    //used to add context on what the valid inputs are for procedures, as well as what the variables in the procedure are
    bool parse_type_mark(token procedure_name_token, token variable_name_token);
    //methods used for parsing one or more parameters in a procedure declaration
    bool parse_parameter_list(token procedure_name_token);
    bool parse_parameter(token procedure_name_token);
