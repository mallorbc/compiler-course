...
`tests/run_golden.py` runs `./compiler` over all 201 test programs in the repo
...
(187) — new subdirectories are picked up automatically. `testPgms/UnitTests/`
...
| --------- | ---------------------------------------- | -------------- |
| `OK`      | exited 0                                 | 98             |
| `ERRORS`  | exited nonzero, terminated normally      | 103            |
...
- **Safety rule:** a default-constructed `Typechecker` now initializes its
  ...
  signature-validator, and Stage 2D statement-check regressions are explicit
  exceptions because those helpers guard a null parent. Do not exercise the
